...
public:
	...
	static const int MAX_CROSSING_EDGE_HASH_SIZE = 1 << 26;		//max buffer size in Storage
	...
	static std::string getItemsFromDir(std::string _path);
	static std::string getSystemOutput(std::string cmd);
	static std::string getExactPath(const char* path);
	static std::string getItesplitmsFromDir(std::string path);
